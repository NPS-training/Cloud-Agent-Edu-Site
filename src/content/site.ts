...
export const demo = {
  lede: "The live portion runs against one repository: NPS-training/Labtainers-Devin, a Docker-based framework of 89 cybersecurity lab exercises written in Python, Bash, Java, and C++. Nobody in the room wrote it, which is the point. Three surfaces are worth seeing first, in this order, because each one answers a question the next one raises.",
      ...
      steps: [
        {
          number: "01",
          label: "Wiki",
          title: "Understand a codebase nobody in the room wrote",
          target: "Devin Wiki",
          minutes: 8,
          prompts: [
          ...
          watchFor:
            "The wiki already exists. It was generated by adding the repository, with no setup and nobody writing documentation. Answers arrive with an architecture diagram and citations that link into real files. On the second prompt it follows the chain rather than guessing: LAB_MASTER_SEED in a lab's config/start.config, the RAND_REPLACE and HASH_REPLACE operators in config/parameter.config, and the per-student seed derived from an email address. Follow-up questions stay on the thread.",
          whyItMatters:
            "This is the first hour of every new assignment, every new billet, and every code base inherited from a contractor. Reading a mechanism out of unfamiliar code is the part of the work that does not compress by adding people.",
        },
        {
          number: "02",
          label: "Review",
          title: "Read a change the way a senior engineer would",
          target: "Devin Review",
          minutes: 7,
          prompts: ["Review this pull request and flag anything that would break an existing lab."],
          watchFor:
            "Comments land inline on the diff, in the same place a human reviewer would leave them, and they cite code elsewhere in the repository that the diff would affect. This is a separate system from the security scan, and it runs on a pull request whether a person or an agent wrote it.",
          whyItMatters:
            "Review is the constraint that appears the moment writing code stops being the constraint. It is also the honest answer to the first question this audience asks, which is who checks the work.",
        },
        {
          number: "03",
          label: "Security",
          title: "Find real vulnerabilities, then fix them",
          target: "Devin Security",
          minutes: 8,
          prompts: ["Run a security review of this repository and rank what you find by severity."],
          watchFor:
            "Findings grouped by severity, with the critical ones first. Turning on auto-fix sends Devin back into the code to open pull requests against the findings. Those pull requests arrive in the review surface from the previous pillar, which is where a person approves or rejects each one.",
          whyItMatters:
      ...
      steps: [
        {
          ...
          whyItMatters:
            "Everything in that window is work a local assistant hands back to you: building, running, reading the error, trying again.",
        },
        {
          number: "05",
          ...
          watchFor:
            "Many sessions running at once in the sessions list, each on its own machine, filtered and pinned as they finish. The shape is map then reduce: parallel workers on identical narrow tasks, with results collected for one review pass.",
          whyItMatters:
  ...
  tierOneClosing:
    "Each of the three raises the question the next one answers: what is in here, is this change sound, and what is already wrong.",
